...
with zipfile.ZipFile("train_V2.csv.zip") as myzip:
    with myzip.open('train_V2.csv') as myfile:
        data_table = pandas.read_csv(myfile)
...
with zipfile.ZipFile("train_V2.csv.zip") as myzip:
    with myzip.open('train_V2.csv') as myfile:
        data_table = pandas.read_csv(myfile, dtype = dtypes_dict)
print(datetime.datetime.now() - start_time)
